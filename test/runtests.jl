...
@testset "gamma and friends" begin
    @testset "gamma, loggamma, and logabsgamma" begin
        ...
        @test lfactorial(2) == logabsgamma(3)
    ...
    @test logabsgamma(3.099) ≅ loggamma(3.099+0im) ≅ 0.786413746900558058720665860178923603134125854451168869796
    @test logabsgamma(1.15) ≅ loggamma(1.15+0im) ≅ -0.06930620867104688224241731415650307100375642207340564554
    @test logabsgamma(0.89) ≅ loggamma(0.89+0im) ≅ 0.074022173958081423702265889979810658434235008344573396963
    @test logabsgamma(0.91) ≅ loggamma(0.91+0im) ≅ 0.058922567623832379298241751183907077883592982094770449167
    @test logabsgamma(0.01) ≅ loggamma(0.01+0im) ≅ 4.599479878042021722513945411008748087261001413385289652419
    ...
    @testset "Other float types" begin
        let x = one(Float16)
            ...
            @test logabsgamma(x) ≈ zero(Float16)
            @test logabsgamma(x) isa Float16
        end
